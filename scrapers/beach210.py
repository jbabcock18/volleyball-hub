...
SOURCE = "210 Beach Sideliners"
URL = "https://210beach.volleyballlife.com/events"
LOCATION = "San Antonio, TX"
_TOURNAMENT_PREFIX = "tournament |"
_LEAGUE_PREFIX = "league |"
_EVENT_PATH_RE = re.compile(r"^/(?:event|events)/(\d+)$", re.IGNORECASE)
...
_TITLE_HINT_RE = re.compile(
    r"\b(tournament|men'?s|women'?s|coed|avp|blind draw|byo|revco|stop|series|triple crown|purse|spring|summer|fall)\b",
    re.IGNORECASE,
)
...
_NON_TOURNAMENT_TITLE_RE = re.compile(
...
def _is_non_tournament_title(value: str) -> bool:
    normalized = normalize_ws(value)
    if not normalized:
        return False
    if _is_tournament_title(normalized):
        return False
    return bool(_NON_TOURNAMENT_TITLE_RE.search(normalized))


def _clean_title(raw: str) -> str:
    title = tidy_title(raw)
    title = re.sub(r"\s*[-|•]\s*VolleyballLife.*$", "", title, flags=re.IGNORECASE)
    title = re.sub(r"\s*[-|•]\s*210 Beach Sideliners.*$", "", title, flags=re.IGNORECASE)
    title = title.strip(" *|-:\t")
    ...
    return bool(
        page.evaluate(
            ...
            () => {
              ...
              const wants = ['next', 'load more', 'show more', 'more events', 'older'];
              for (const el of candidates) {
                ...
                const likely =
                  wants.some((needle) => text.includes(needle) || aria.includes(needle)) ||
                  cls.includes('next') ||
                  rel.includes('next');
                if (!likely) continue;
...
def scrape() -> list[Tournament]:
    try:
        ...
        raise ScrapeDependencyError(
            "Playwright is required for 210 Beach Sideliners scraping. Install dependencies and run 'playwright install chromium'."
        ) from exc
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            raw_items = _merge_items(dom_items, api_items, fallback_items)
        finally:
            browser.close()
    ...
    with sync_playwright() as pw:
        ...
        try:
            for item in raw_items:
                ...
                title = _select_best_title([item.get("text", ""), list_context])
                ...
                detail_candidates.extend(_parse_json_ld_titles(detail_json_ld))
                detail_title = _select_best_title(detail_candidates)
                if detail_title:
                    title = detail_title
                ...
                has_tournament_title = _is_tournament_title(title) or any(_is_tournament_title(c) for c in detail_candidates)

                if has_league_label and not (has_tournament_label or has_tournament_title):
                    continue
                if _is_non_tournament_title(title) and not (has_tournament_label or has_tournament_title):
                    continue

                jsonld_date = _parse_json_ld_date(detail_json_ld)
                if jsonld_date:
                    date_value = jsonld_date
                elif date_value is None and detail_body:
